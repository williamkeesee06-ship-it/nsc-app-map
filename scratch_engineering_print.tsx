// EngineeringPrintOverlay ΓÇö Phase 7: renders a single EngineeringPrint as a
// GroundOverlay on the workspace map, with optional NW/SE drag handles for
// alignment. Visual reference only ΓÇö never used to derive billing.
...
export default function EngineeringPrintOverlay({ print, editing, onCornersChange }: Props) {
  ...
  useEffect(() => {
    ...
    rotateMarkerRef.current = new google.maps.Marker({
      ...
      position: ne,
      draggable: true,
      label: { text: "Γå╗", fontSize: "12px", fontWeight: "bold", color: "#fff" },
      icon: {
        path: google.maps.SymbolPath.CIRCLE,
        scale: 9,
        fillColor: "#39ff7a",
