...
Synthesized from convo.pdf (46 pages) — every locked decision quoted/paraphrased faithfully.

## 1. Three-Level Product Architecture

The unified product has **three levels**, not two:

1. **Map View** — quick visual overview of the job, just the gist (jobs map for discovery + filtering).
2. **Workspace (Focused Job Editor)** — full rendered as-built picture for one job.
3. **Layer/Detail Controls** — drill into specific dates or people inside one job.

> "Open on map → jobs filtered by signed-in user role → open a job and always render existing work → allow new as-built creation when nothing exists → support map/satellite views → support optional engineering print overlays → support layered daily records under one job → roll up billable units to the job level."
...
- **Job** — Smartsheet-backed record (the existing 191-job catalog).
- **Print overlays** — optional reference layers tied to the job (PDF/image, manually aligned to the map).
- **As-built sessions / layers** — dated work packages, one per foreman per date.
- **Features** — lines, points, notes, photos, labels, units inside a specific as-built layer.
...
- **v1: print overlay is visual reference only — NEVER the source of truth for measurements or billing.** Billing/geometry come from drawn as-built objects.
- **Engineering Print badge** — only ONE active per job at a time. Reassignable to a revised print. Older prints remain attached unless deleted. UX: "Set as Engineering Print" button on a PDF attachment.
...
### 4.1 Job card on first click — fields and layout
...
- The map shows the **Quick Reference Layer** by default — NOT every granular as-built object.
...
- Visual only — no footage, no billing units.
...
- First as-built saved for a job → **auto-generate the reference layer gist**.
- Later edits flag the gist as "out of date" until user manually syncs from the map-view job card.
- Job card sync icon states:
  - No reference layer yet → (auto-creates on first as-built save)
  - Out of date → red mark
...
- **Map auto-centers on the job's real location** when opened (random reset breaks flow — explicit rule).
- Other jobs are hidden.
- Full toolset stays available.
- Layers and live totals shown immediately so the user is ready to work.

### 5.2 Layout
- **Top:** thin job context strip (work order, address, status) — stays light.
...
- **Older layers are NOT editable by default** — they become editable only when explicitly toggled (deliberate layer-edit mode).
...
### 6.1 Panel structure (top → bottom)
1. **Top: Job totals / billable summary**
2. **Middle: Layer list by foreman + date**
3. **Under each layer: grouped map objects** (Cable, Removed Cable, Poles, MH, HH, PED, Notes, etc.) — counts/footage shown beside each group name. Not a flat list of every feature.

### 6.2 Totals behavior
- Active Layer Totals (today's work)
- Job Totals (running grand total)
- Both visible at once — user shouldn't flip views.
...
Existing tools (already built — DO NOT REBUILD): cable family, point tools (Pole, HH, MH, PED, Cabinet, Anchor), measure, edit, billable flow, persistence. Occupied conduit adder is already correct.
...
### 7.2 Status → color (universal across both Quick and As-Built modes)
- NEW: red
...
- Same core tool family exists in both modes — different prompt depth + behavior.
...
1. **Job lookup and map load** — work order lookup, address fill, geocode, map capture always work.
2. **Canvas stability** — draw, select, move, delete, undo/redo, zoom, pan, background image behavior.
3. **Billing integrity** — every existing tool writes the right units into `state.unitMap`, removes stale entries on delete, preserves decimals (no Math.round).
4. **Export and persistence** — save/load and PDF export produce clean, submission-ready output every time.
