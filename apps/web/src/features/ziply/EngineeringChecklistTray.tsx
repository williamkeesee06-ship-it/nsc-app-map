...
import type { Job, DrawingContextState } from "@nsc/types";
  ...
  const { placedTerminals, unplacedTerminals, placedCables, unplacedCables } = useMemo(() => {
    ...
    terminals.forEach(t => {
      ...
      const isPlaced = drawnObjects.some(
        obj => (obj.type === "ziply_terminal" || obj.type === "ziply_ped") && obj.style?.userLabel === t.name
      );
    ...
    cables.forEach(c => {
      ...
      const isPlaced = drawnObjects.some(
        obj => 
          obj.style?.ziplyFootage === Math.round(c.lengthFeet || 0) &&
          (c.cableType ? obj.style?.ziplyCableType === c.cableType : true)
      );
      ...
      <div style={{ maxHeight: 200, overflowY: "auto", display: "flex", flexDirection: "column", gap: 4 }}>
        ...
        {unplacedCables.map((c, i) => (
          <div key={`uc-${i}`} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 10, color: "#475569" }}>
            <span style={{ width: 12, height: 12, borderRadius: 2, border: "1px solid #cbd5e1" }} />
            <span>{c.buildType === "bore" ? "Bore" : "Cable"} - {Math.round(c.lengthFeet || 0)}'</span>
          </div>
        ))}
