...
// Fast in-memory cache to make page preview thumbnails load instantly (0ms latency)
...
export async function getBlueprintImage(id: string): Promise<string | null> {
  ...
  try {
    ...
    if (result) {
      imageMemoryCache.set(id, result);
    }
    return result ?? null;
  } catch {
    return null;
      ...
      tx.oncomplete = () => {
        ...
        keys.forEach((k, i) => {
          if (typeof vals[i] === "string" && vals[i]) {
            ...
            out.set(keyStr, vals[i]);
            imageMemoryCache.set(keyStr, vals[i]);
          }
        });
        resolve();
      };
